...
/**
 * Token 有效期状态（由当前用户信息接口响应解析并缓存在本地）
 *
 * 后端可在该接口响应中返回可选字段 `token_expires_at`（ISO 8601 字符串）；
 * 字段缺失、为 null 或无法解析时，一律视为永久有效。
 */
export interface TokenStatus {
  /** ISO 8601 过期时间；null 表示永久有效 */
  expires_at: string | null;
  /** 最近一次向后端查询该状态的时间（ISO 8601） */
  checked_at: string;
}
