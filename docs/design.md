...
- NewShop：图片 `categories` 由 `media_content_type` 判断，非 `video` 为 `1`，`video` 为 `2`。
    ...
    "variants": [
      {
        ...
        "main_sku": "ABC",          // (采) NOT NULL；= sku 按 `-` 切前段，无 `-` 取整体，无值存 ""
        "option1": "Black",         // (采) NOT NULL
        "option2": "M",             // (采)
        "option3": null,            // (采)
        ...
        "weight": null,             // (采)
        "weight_unit": null,        // (采)
        "taxable": 1,               // (采) NOT NULL
        "tax_code": null,           // (采)
        "presentment_prices": [...] // (采) 多币种价格（JSON）
      }
    ],
...
>
> **`main_sku` 规则**：取 `sku` 第一个 `-` 之前的内容；`sku` 无 `-` 则取整个 `sku`；`sku` 为空则存 `""`。
>
...
```jsonc
{
  ...
  "platforms": {                    // 平台开关
    "shopify": true, "tiktok": true, /* ... */
  },
  "retry": { "max_attempts": 5, "backoff_base_ms": 2000,
...
| 对象 | 字段 |
|---|---|
| Product | `body_html`、`handle`、`options`、`product_type`、`published_scope`、`tags`、`title`、`vendor` |
| Variant | `barcode`、`compare_at_price`、`grams`、`option1`、`option2`、`option3`、`presentment_prices`、`position`、`price`、`sku`、`taxable`、`tax_code`、`title`、`weight`、`weight_unit`，提交时额外带 `image_id` |
| Image | `position`、`src`、`alt`，NewShop 额外带 `categories` |
...
| NewShop | `title/post_content/tags/variant_attrs/slug/content` → `title/body_html/options/handle`；`handle=slug`；`body_html=post_content ?? content ?? ""` | `variant_attrs`：`position=序号`，`values=value`，删除 `value/is_visible/is_variation/is_taxonomy`；无规格则默认 `Title / Default Title` | `gallery`：`id=ID`，`src=url`，`categories=(media_content_type !== "video" ? 1 : 2)` | `variants`：`image_id=feature_image.ID`，`compare_at_price=regular_price`，`option1/2/3` 从 `attrs` 按 option name 匹配；无 variants 时用商品 `price/sku/regular_price` 生成 Default Title；`main_sku=sku` 第一个 `-` 前 50 字符；`compare_at_price` 空或低于 `price` 时置 `0` |
| ShopBase | `description` → `body_html`，另取 `handle/product_type/published/published_scope/tags/title/vendor` | `options[].values[].name` 转为 values；规格值 ID 在 variant 中反查成名称 | `images[]` 取 `alt/src`，position 重排，保留源 image id 做映射 | `weight=0`，`weight_unit=g`；`option1/2/3` 从 option value id 反查名称；`image_id` 映射 |
| ShopLine | `spu.title` → `title`；详情优先 DOM `.mce-content-body`，否则 `productSeo.desc`；`handle` 取 `productSeo.url` 最后一段 | `sku.skuAttributeMap`：`name=defaultName`，`position=attributeWeight`，`values=skuAttributeValueMap.defaultValue`，按 position 排序 | `spu.imageList`：`src=url`，`alt=alt`，position 重排 | `sku.skuList`：`price/originPrice` 分为单位，除以 100；`sku=itemNo`；`option1/2/3` 按 `skuAttributeIds.attributeWeight` 匹配；`weightUnit` 默认 `g`；单 variant 时强制 `Title / Default Title` |
| XShopPy | `/buyer/product/pop-detail` 成功后取返回 `data` 的 Product 白名单字段；`body_html` 清洗 `data-original` 图片 | `attribute`：`name=specName`，`values=specItems`，`position=序号`，删除 `specName/specItems/specCodes`；无规格则默认 `Title / Default Title` | `images[].file_preview` → `src`，position 重排 | `sku_list`：`sku=sku_code`；`spec` JSON 按 option name 填 `option1/2/3`；无规格则 `Default Title` |
| ShopLazza | API `data.product` 取 Product 白名单；`body_html` 优先 DOM `.product-info__desc-tab-content` 清洗结果，否则 `meta_description` | 删除 option 的 `id`；无规格则默认 `Title / Default Title` | API 图片 `src` 前补 `https:`，position 重排 | `weight` 空则 `0`；单 variant 且空 `option1` 时设 `Default Title`；variant image 按 `https:` + `image.src` 映射到图片 |
| TikTok | `product_model.name` → `title`；`handle` 从 URL 的 `product/products/pdp` 后一段取，若是 18-20 位纯数字则用 title slug；`body_html` 由 `product_properties` 表格 + `description` block HTML 组合 | `sale_properties`：`name=property_name`，`values=property_values[].property_value_name`，position 重排 | `product_model.images[].url_list[0]` → `src`，去掉 query，position 重排 | `skus`：`sku=sku_name`；`property_pairs[].sku_property_value_name` → `option1/2/3`；title 用规格值 ` / ` 拼接；价格从 `promotion_model.promotion_product_price.skus_price` 取 `seller_subtotal_deduction`；`compare_at_price=price / discount_decimal`；无有效规格时生成 `Default Title` |
...
40. ✅ **main_sku**：`sku` 按第一个 `-` 切前段；无 `-` 取整个 `sku`；无值存 `""`。
41. ✅ **images.categories**：`1=图片`，`2=视频`。
