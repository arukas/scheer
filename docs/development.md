...
「外部配置导入」功能要求接入方配置稳定的 Extension ID。

**商店渠道（正式）：扩展已上架 Chrome 应用商店，商店版本的 ID 由商店签名固定，同一条目更新版本 ID 不变，无需任何额外配置。** 接入方直接使用商店 ID（可从商店详情页 URL 看到：`.../detail/<id>`）。

**zip 私分发 / 开发渠道（可选）**：加载已解压扩展时 ID 默认由安装路径决定，换个目录就会变。如需固定，可通过 manifest `key` 注入：
...
- **商店版与带 key 的 zip 版 ID 不同**。接入方配置的 ID 必须与用户实际安装渠道一致；正式对外一律以商店 ID 为准。
- 上传商店的构建不要注入 `SCHEER_EXTENSION_KEY`（商店会用自己的签名固定 ID，保持商店包 manifest 干净）。
- 一旦发布过带 key 的 zip 构建，后续 zip 版本必须使用同一把 key，否则用户安装后 ID 变化、接入方配置会失效。
